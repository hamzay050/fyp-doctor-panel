  ...
  return (
    <>
      <Box display="flex" alignItems="center" gap={1} p={2}>
        <AvatarBox
          image={patientData?.profileDetails[0]?.profilePicture}
          mheight={height}
          mwidth={width}
        />
        <Typography variant="body1" sx={{ fontWeight: "570" }}>
          {patientData?.profileDetails[0]?.firstName +
            " " +
            patientData?.profileDetails[0]?.lastName}
        </Typography>
        <Typography variant="body2">
          {patientData?.profileDetails[0]?.dateOfBirth
            ? moment().diff(
                new Date(
                  patientData.profileDetails[0].dateOfBirth
                ).toLocaleDateString(),
