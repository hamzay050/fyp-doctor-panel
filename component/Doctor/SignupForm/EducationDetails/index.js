...
export default function EducationDetails() {
  ...
  const newEducation = () => {
    if (education.title !== '' && education.institute !== '' && education.startDate !== '' && education.endDate !== '') {
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2rem 0 0 0' }}>
      <Autocomplete
        ...
        onChange={(_, newValue) => setEducation((prev) => ({ ...prev, title: newValue }))}
        size="small"
        getOptionLabel={(option) => option}
        filterSelectedOptions
        sx={{ width: { xs: '100%', sm: '40%', md: '80%' }, margin: { xs: '0.4rem 0', sm: '0.4rem', md: '0 0.4rem' } }}
        renderInput={(params) => <TextField {...params} label="Degree Title" placeholder="Degree Title" />}
      />
      ...
      <Box sx={{ display: 'flex', justifyContent: 'flex-end', width: '80%', marginTop: '1rem' }}>
        <Button variant="contained" size="small" onClick={newEducation} color="secondary" sx={{ width:'15%', color: 'white' }}>
          Add
        </Button>
      </Box>
